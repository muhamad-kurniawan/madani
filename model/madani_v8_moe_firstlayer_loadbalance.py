...
class MoEFeedForwardTop2(nn.Module):
    """
    MoE feed-forward module with top-2 gating and load balancing regularization.
    ...
    def forward(self, x):
        ...
        load_loss = num_experts * (importance**2).sum() / (importance.sum()**2 + 1e-10)
...
class CustomTransformerEncoderFFLayer(nn.Module):
    ...
    Returns a tuple: (output, auxiliary_loss) where aux_loss is zero.
    """
    ...
    def forward(self, src, src_mask=None, src_key_padding_mask=None):
        ...
        src = self.norm2(src)
        return src, 0.0  # Standard layer: no auxiliary loss.
...
# ---------------------
# Mixed Encoder: First layer is MoE, remaining layers are standard.
...
# ---------------------
# Encoder MoE - now using MoE only in the FIRST layer.
# ---------------------
class EncoderMoE(nn.Module):
    def __init__(self, d_model, N, heads, frac=False, attn=True,
        ...
        if self.attention:
            # Use our mixed encoder that applies MoE only on the first layer.
