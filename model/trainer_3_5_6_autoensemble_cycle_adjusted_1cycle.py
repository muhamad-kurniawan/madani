...
class ModelTrainer:
    ...
    def fit(self,
        ...
        if pretrain_epochs > 0:
            print(f"Starting pre-training phase for {pretrain_epochs} epoch(s) with 1cycle policy...")

            # 1cycle parameters:
            base_lr = alpha2             # starting learning rate
            factor = 10                  # determines maximum lr (here: 10 * base_lr)
            max_lr = base_lr * factor    # peak learning rate
            final_lr = base_lr / 100     # ending learning rate after the cycle
            total_steps = pretrain_epochs * len(self.train_loader)
            step = 0
            ...
            for epoch in range(pretrain_epochs):
                ...
                for i, data in enumerate(self.train_loader):
                    ...
                    # Use a two-phase schedule: increasing phase (first 30%) and decreasing phase (remaining 70%)
                    if pct < 0.3:
                        lr_current = base_lr + (max_lr - base_lr) * (pct / 0.3)
                    else:
                        lr_current = max_lr - (max_lr - final_lr) * ((pct - 0.3) / 0.7)
                    # Update learning rate in the optimizer
                    for group in self.optimizer.param_groups:
                        group['lr'] = lr_current

                    # Pretrain iteration as before:
                    X, y, formula = data
                    ...
                    step += 1
                dt = time() - t0
                print(f"[Pretrain] Epoch {epoch+1}/{pretrain_epochs}, took {dt:.2f}s, lr={lr_current:.6f}")
            print("Pre-training phase completed with 1cycle policy.\n")
