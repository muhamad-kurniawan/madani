...
class RBFEncoder(nn.Module):
    ...
    If trainable_centers=True, the RBF centers are learned.
    If trainable_gamma=True, the gamma parameter is learned.
    """
    def __init__(self,
        ...
        # -- Initialize centers in [0,1]
        init_centers = torch.linspace(0, 1, num_centers).view(-1, 1)  # shape [num_centers, 1]

        if trainable_centers:
            # Each center will be an nn.Parameter
            self.centers = nn.Parameter(init_centers, requires_grad=True)
        else:
            # Keep them as a buffer (non-trainable)
    ...
    def forward(self, x):
        ...
        if self.log10:
            ...
            # Or at least clamp to avoid frac=0
            frac = torch.clamp(frac, min=1e-8, max=1.0)

        # Expand frac to shape [B, T, 1]
        frac = frac.unsqueeze(-1)  # [B, T, 1]

        # 2) Compute distance to each center
        # centers shape = [num_centers, 1]
        # We'll do frac - centers^T => shape [B, T, num_centers]
        diff = frac - self.centers.view(1, 1, self.num_centers).to(frac.device)

        # 3) Squared distance & RBF
        dist2 = diff.pow(2)  # [B, T, num_centers]
        # gamma is shape [1], so broadcast automatically
        phi = torch.exp(-self.gamma * dist2)  # [B, T, num_centers]

        # 4) Linear projection to rbf_dim
...
class EncoderMoE(nn.Module):
    def __init__(self,
        ...
        # RBFEncoder with trainable gamma & centers
        self.pe = RBFEncoder(
            ...
            log10=False,
            compute_device=self.compute_device,
            trainable_centers=True,  # <--- enable learnable centers
            trainable_gamma=True     # <--- enable learnable gamma
        )
        # (Optionally another for log-scale if you like):
        self.ple = RBFEncoder(
    ...
    def forward(self, src, frac):
        ...
        pe_out = self.pe(frac) * pe_scaler
        ple_out = self.ple(frac) * ple_scaler

        x_src = x + pe_out  # or combine ple_out if you like
        if self.attention:
