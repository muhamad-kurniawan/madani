...
def build_rope_frequencies(max_seq_len, head_dim, base=10000.0):
    """
    Build a [max_seq_len, head_dim/2] tensor of rotation angles for RoPE.
    Standard formula: theta_{pos, i} = pos / (base^(2i/head_dim)).
    """
    ...
    # denominator
    freq_div = base ** (2 * i / head_dim)  # shape [half_dim]
...
def rope_rotate(x, rope_freq):
    """
    Rotate x using the RoPE angles in rope_freq.
    x:          [B, nhead, T, head_dim]
    rope_freq:  [T, head_dim/2], for T positions and half-dim
    """
    B, nhead, T, d = x.shape
    half = d // 2
    ...
    # freq_cos/sin => shape [T, half] => broadcast to [1, 1, T, half]
    freq_cos = torch.cos(rope_freq).unsqueeze(0).unsqueeze(0)
    freq_sin = torch.sin(rope_freq).unsqueeze(0).unsqueeze(0)
...
class CustomMultiHeadAttention(nn.Module):
    def __init__(self, d_model, nhead, dropout=0.1, max_seq_len=4096):
        ...
        # ---- RoPE buffer ----
        # Precompute angles for up to max_seq_len
    ...
    def forward(
        ...
        # We slice the rope_freq for the current T (positions 0..T-1)
        cur_rope_freq = self.rope_freq[:T]  # shape [T, head_dim/2]
        Q = rope_rotate(Q, cur_rope_freq)
...
class CustomTransformerEncoderMoELayer(nn.Module):
    def __init__(self, 
                 ...
                 num_experts=4,
                 gating_noise=0.0,
                 max_seq_len=4096):   # <--- add a parameter for max_seq_len
        super().__init__()
    ...
    def forward(self, src, src_mask=None, src_key_padding_mask=None):
        # 1) Self-attention (with RoPE inside)
    ...
    def __init__(self,
        ...
        if self.attention:
            encoder_layer = CustomTransformerEncoderMoELayer(
                ...
                num_experts=num_experts,
                gating_noise=gating_noise,
                max_seq_len=max_seq_len   # pass forward
            )
