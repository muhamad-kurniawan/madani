...
class MoEFeedForwardTop1(nn.Module):
    ...
    Each token's output is routed to exactly one expert.
    """
    def __init__(self, 
                 d_model, 
    ...
    def forward(self, x):
        ...
        # 2) Inject gating noise (only during training)
        ...
        # top1_scores:  [B, T, 1] (the gating probability for the top expert)
        # top1_experts: [B, T, 1] (the expert index)

        # 5) Initialize output buffer
        out = torch.zeros_like(x)

        # 6) For the single selected expert, gather tokens, forward them, and scatter back
        # top1_experts has shape [B, T, 1], but we can just index by [:, :, 0]
        expert_idx = top1_experts[..., 0]    # [B, T]
        gating_weight = top1_scores[..., 0]  # [B, T]

        for e_idx in range(self.num_experts):
            # tokens assigned to expert e_idx
            mask = (expert_idx == e_idx)  # [B, T] bool
            if mask.any():
                # gather only those tokens
