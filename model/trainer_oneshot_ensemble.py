...
class ModelTrainer:
    def __init__(self,
                 ...
                 n_elements=8,
                 ensemble_size=1,   # set ensemble_size > 1 for oneshot ensemble training
                 capture_every=None,
                 verbose=True,
                 drop_unary=True,
                 scale=True):
        ...
        self.ensemble_size = ensemble_size  # ensemble parameter
    ...
    def fit(self, epochs=None, checkin=None, losscurve=False, discard_n=10, feature_selection_phase=0.5):
        ...
        for epoch in range(epochs):
            ...
            for i, data in enumerate(self.train_loader):
                ...
                # Check if we are using an ensemble
                if self.ensemble_size == 1:
                    ...
                    # Expecting output shape: (batch_size, ensemble_size*2)
                    output = output.view(batch_size, self.ensemble_size, 2)
                    predictions = output[:, :, 0]
                    uncertainties = output[:, :, 1]
                    loss = 0
                    for j in range(self.ensemble_size):
                        loss += self.criterion(predictions[:, j].reshape(-1),
                                               uncertainties[:, j].reshape(-1),
                                               y.reshape(-1))
        ...
        with torch.no_grad():
            for (X, y, formula) in loader:
                ...
                if self.ensemble_size == 1:
                    ...
                    output = output.view(batch_size, self.ensemble_size, 2)
                    predictions = output[:, :, 0]
                    log_uncerts = output[:, :, 1]
                    if self.classification:
                        predictions = torch.sigmoid(predictions)
                        uncertainties = torch.sigmoid(log_uncerts)
                    else:
                        uncertainties = torch.exp(log_uncerts) * self.scaler.std
                        predictions = self.scaler.unscale(predictions)
                    prediction_out = predictions.mean(dim=1)
                    uncertainty_ = uncertainties.mean(dim=1)
...
if __name__ == '__main__':
    ...
    # Also, set ensemble_size to a value > 1 for oneshot ensemble training.
    model = CrabNet(out_dims=3, d_model=512, N=3, heads=4, feature_selector_k=200)
    print(model)
    
    trainer = ModelTrainer(model, model_name='CrabNet', n_elements=8, ensemble_size=5)
