...
class Embedder(nn.Module):
    ...
    def forward(self, src, frac):
        ...
        # Multiply fractional stoichiometry with the embedding
        weighted_emb = x_emb * frac.unsqueeze(-1)
        # Concatenate the original embedding with the weighted embedding
        concatenated_emb = torch.cat((x_emb, weighted_emb), dim=-1)
...
class EncoderMoE(nn.Module):
    def __init__(self, d_model, N, heads, attn=True, compute_device=None, num_experts=4, gating_noise=0.0):
        super().__init__()
        self.d_model = d_model
        self.N = N
        self.heads = heads
        ...
        # Updated Embedder to process fractional embeddings
        ...
        if self.attention:
            encoder_layer = CustomTransformerEncoderMoELayer(
                d_model=self.d_model * 2,  # Account for concatenated embeddings
                nhead=self.heads,
                dim_feedforward=2048,
                dropout=0.1,
                layer_norm_eps=1e-5,
    ...
    def forward(self, src, frac):
        ...
        hmask = (frac == 0).unsqueeze(-1).repeat(1, 1, x.shape[-1])
        x = x.masked_fill(hmask, 0)
        return x


class Madani(nn.Module):
    def __init__(self, out_dims=3, d_model=512, N=3, heads=4, compute_device=None, residual_nn='roost', num_experts=4, gating_noise=0.1):
        super().__init__()
        self.avg = True
        ...
        # Adjusted encoder to handle new embedding format
        ...
        if residual_nn == 'roost':
            ...
            self.output_nn = ResidualNetwork(self.d_model * 2, self.out_dims, self.out_hidden)  # Adjust for concatenation
        else:
