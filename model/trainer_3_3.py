...
# =============================================================================
# ModelTrainer (as provided) with multi-stage training (stage 2 includes penalties)
# =============================================================================
...
class ModelTrainer:
    ...
    def multi_stage_fit(self, stage1_epochs=50, stage2_epochs=100, stage3_epochs=50, checkin=5, losscurve=True, discard_n=10, feature_selection_phase=0.5):
        ...
        Multi-stage training:
         ...
         - Stage 2: Introduce feature selection gradually (e.g. selector_scale = 0.5)
        ...
        # Hyperparameters for penalty terms (adjust as needed)
        ...
        # Re-enable the feature selector and set a modest scaling (e.g., 0.5)
        if hasattr(self.model.encoder.embed, 'selector_scale'):
...
# =============================================================================
# Ensemble Training: Model Averaging & Stability Selection on Stage 2
# =============================================================================

def ensemble_training(ensemble_runs=5, stage1_epochs=50, stage2_epochs=100, stage3_epochs=50):
    """
    Train multiple instances of the model (with different seeds or batch orders)
    during the multi-stage training. For each run record the final (hard) feature mask,
    and later compute the frequency each feature is selected (stability selection).
    Also, aggregate predictions via model averaging.
    """
    # To accumulate frequency counts for each feature:
    feature_selection_counts = None
    # To accumulate predictions from each ensemble member:
    ensemble_predictions = []
    # (Assume training and validation CSV file names are given.)
    train_file = 'your_training_data.csv'
    val_file = 'your_validation_data.csv'
    
    for run in range(ensemble_runs):
        print(f"\n--- Ensemble Run {run+1} / {ensemble_runs} ---")
        # Set different seed for each run
        seed = 42 + run
        torch.manual_seed(seed)
        np.random.seed(seed)
        # Initialize a new instance of the model (CrabNet should be defined elsewhere)
        from your_model_file import CrabNet  # adjust import as needed
        model = CrabNet(out_dims=3, d_model=512, N=3, heads=4, feature_selector_k=200)
        trainer = ModelTrainer(model, model_name=f'CrabNet_run{run}', n_elements=8)
        trainer.load_data(train_file, batch_size=512, train=True)
        trainer.load_data(val_file, batch_size=512, train=False)
        trainer.multi_stage_fit(stage1_epochs=stage1_epochs,
                                stage2_epochs=stage2_epochs,
                                stage3_epochs=stage3_epochs,
                                checkin=5,
                                losscurve=True,
                                discard_n=500)
        # After training, record the final hard feature mask
        if trainer.model.encoder.embed.feature_selector is not None:
            hard_mask = trainer.model.encoder.embed.feature_selector.hard_mask().detach().cpu().numpy()  # shape (feat_size,)
            if feature_selection_counts is None:
                feature_selection_counts = hard_mask
            else:
                feature_selection_counts += hard_mask
        # Get predictions on the validation data
        _, pred, _, _ = trainer.predict(trainer.data_loader)
        ensemble_predictions.append(pred)
    
    # Average feature selection frequencies over ensemble runs
    consensus_feature_selection = feature_selection_counts / ensemble_runs
    # For example, choose features selected in at least 70% of runs as “stable”
    stable_features = np.where(consensus_feature_selection >= 0.7)[0]
    print("\nConsensus feature selection frequency (per feature):")
    print(consensus_feature_selection)
    print("Stable features (selected in >=70% of runs):", stable_features)
    
    # Also perform model averaging: average predictions over ensemble runs
    avg_prediction = np.mean(ensemble_predictions, axis=0)
    print("\nAveraged predictions from ensemble:")
    print(avg_prediction)
    # Return the consensus mask and averaged predictions for further evaluation.
    return consensus_feature_selection, stable_features, avg_prediction
...
if __name__ == '__main__':
    # For a single training run you could simply create and print your model:
    from your_model_file import CrabNet  # Ensure that CrabNet is imported from your model module
    model = CrabNet(out_dims=3, d_model=512, N=3, heads=4, feature_selector_k=200)
    print(model)
    
    # To run ensemble training (which includes model averaging and stability selection),
    # uncomment and adjust the following lines with the correct data file paths:
    #
    # consensus_mask, stable_feats, avg_preds = ensemble_training(
    #     ensemble_runs=5,
    #     stage1_epochs=50,
    #     stage2_epochs=100,
    #     stage3_epochs=50
