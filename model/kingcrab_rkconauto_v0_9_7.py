...
class GlobalRankedFeatureSelector(nn.Module):
    ...
    """
    def __init__(self, input_dim, k, init_temp=5.0, final_temp=0.01, cycle_length=5):
        """
        Args:
    ...
    def update_temperature(self, epoch):
        ...
        # Decay factor for maximum temperature (modify gamma as needed)
        gamma = 0.95  
        decayed_init_temp = self.init_temp * (gamma ** cycle)
        T = epoch % self.cycle_length
        self.current_temp = self.final_temp + 0.5 * (decayed_init_temp - self.final_temp) * (1 + math.cos(math.pi * T / self.cycle_length))
