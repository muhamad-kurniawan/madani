...
###############################################################################
# MoE Feed-Forward module with Adaptive/Dynamic Capacity Routing (without load balancing)
###############################################################################
class MoEFeedForwardAdaptiveNoLB(nn.Module):
    """
    MoE feed-forward module that dynamically selects a variable number of experts per token
    based on a cumulative gating threshold (p_threshold) and an optional maximum number (k_max).
    No load balancing loss is computed.
    
    Returns:
      - out: The combined output from the selected experts.
      - A dummy auxiliary loss of 0.0 (for compatibility).
    """
    ...
    def forward(self, x):
        ...
        # Adaptive selection:
        # 1. Sort gating scores in descending order.
        sorted_scores, sorted_indices = torch.sort(gating_scores, dim=-1, descending=True)  # [B, T, num_experts]
        cumsum_scores = sorted_scores.cumsum(dim=-1)  # [B, T, num_experts]

        # 2. Create a mask: select experts until cumulative sum exceeds p_threshold.
        mask_sorted = (cumsum_scores <= self.p_threshold)
        mask_sorted[..., 0] = True  # ensure at least one expert is selected.

        # 3. Enforce maximum number of experts (k_max).
        ...
        # 4. Unsort: create a mask in the original expert order.
        ...
        # 5. Zero out unselected experts and normalize effective gating weights.
        effective_gating = gating_scores * selection_mask  # [B, T, num_experts]
        effective_sum = effective_gating.sum(dim=-1, keepdim=True) + 1e-10
        effective_gating = effective_gating / effective_sum

        # 6. Route tokens through each expert.
        ...
        return out, 0.0  # Return dummy auxiliary loss 0.0
...
class CustomTransformerEncoderFFLayer(nn.Module):
    ...
    def forward(self, src, src_mask=None, src_key_padding_mask=None):
        ...
        src = self.norm2(src)
        return src, 0.0  # No auxiliary loss.
...
class CustomTransformerEncoderMoELayer(nn.Module):
    """
    A Transformer Encoder layer that uses an MoE feed-forward block with adaptive routing.
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
        ...
        super().__init__()
        self.self_attn = CustomMultiHeadAttention(d_model, nhead, dropout=dropout)
        self.feed_forward = MoEFeedForwardAdaptiveNoLB(
            d_model=d_model,
...
# It accumulates auxiliary losses (dummy zeros from standard layers).
###############################################################################
