...
###############################################################################
# Differentiable Selection: Gumbel-softmax based attention
###############################################################################
class DiffSelectMultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, key_padding_mask=None, attn_mask=None):
        ...
        # Apply along the key dimension (last dimension) for each query.
...
###############################################################################
# Updated Transformer Layers using DiffSelectMultiHeadAttention
###############################################################################
class CustomTransformerEncoderFFLayer(nn.Module):
    ...
    Here, self-attention uses differentiable selection.
    """
...
class CustomTransformerEncoderMoELayer(nn.Module):
    ...
    The self-attention uses differentiable selection.
...
class CustomTransformerEncoderMixedLastMoE(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
        ...
        # Create N-1 standard layers using DiffSelectMultiHeadAttention.
...
###############################################################################
# Encoder MoE - uses MoE only in the LAST layer
###############################################################################
class EncoderMoE(nn.Module):
    ...
    def __init__(self, d_model, N, heads, frac=False, attn=True,
                 compute_device=None, num_experts=4, gating_noise=0.0, tau=0.3):
        super().__init__()
        self.d_model = d_model
        self.N = N
        self.heads = heads
