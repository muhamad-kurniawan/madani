...
class Madani(nn.Module):
    ...
    def forward(self, src, frac):
        ...
        if self.avg:
            # Mask the invalid tokens to zeros
            output = output.masked_fill(mask, 0)
            
            # ---------------------------------------------------------
            # (NEW)  Produce per-token gating weights from 'output'
            #         shape of 'output' is [B, T, out_dims].
            #         Let's call the gating logits: gating_logits
            # ---------------------------------------------------------
            gating_logits = self.token_gater(output)  # [B, T, 1]

            # We want the gating weights in [B, T]. Squeeze the last dim
            gating_logits = gating_logits.squeeze(-1)  # [B, T]

            # Mask out invalid positions (where mask is True)
            #  We can set them to a very negative number so softmax -> 0
            gating_logits = gating_logits.masked_fill(mask[..., 0], float('-inf'))

            # Softmax across tokens dimension => gating_weights in [B, T]
            gating_weights = torch.softmax(gating_logits, dim=1)  # [B, T]

            # Expand to multiply [B, T, out_dims]
            gating_weights_expanded = gating_weights.unsqueeze(-1)  # [B, T, 1]

            # Weighted sum
            weighted_sum = (output * gating_weights_expanded).sum(dim=1)  # [B, out_dims]

            # ----------------------------------------------------------
            # If your final half-splitting is still needed (chunk(2,-1)),
            # you must do it on the final aggregated representation.
            # That implies out_dims is 2x bigger than "real" output size.
            # e.g. if out_dims=6, chunk(2) => (3,3).
            #
            # So you might do:
            # ----------------------------------------------------------
            weighted_sum, logits = weighted_sum.chunk(2, dim=-1)
            probability = torch.sigmoid(logits)
            weighted_sum = weighted_sum * probability

            # This 'weighted_sum' is your final [B, out_dims/2]
            output = weighted_sum
