...
# ----------------------------
# Dynamic PMA Block (Dynamic Query Generation in Pooling by Multihead Attention)
# ----------------------------
class DynamicPMA(nn.Module):
    """
    Dynamic PMA block that uses dynamic query generation.
    Instead of fixed seed vectors, it generates query vectors from an aggregated
    ...
    def __init__(self, d_model, n_heads, num_seeds):
        super(DynamicPMA, self).__init__()
        self.num_seeds = num_seeds
        # Query generator: generates num_seeds queries from aggregated context
        self.query_gen = nn.Sequential(
            nn.Linear(d_model, d_model),
            nn.ReLU(),
            nn.Linear(d_model, num_seeds * d_model)
        )
    ...
    def forward(self, X):
        ...
        batch_size = X.size(0)
        # Aggregate input set (using mean pooling) to obtain a context vector.
        context = X.mean(dim=1)  # (batch, d_model)
        # Generate dynamic query vectors from the context.
        Q = self.query_gen(context)  # (batch, num_seeds*d_model)
        Q = Q.view(batch_size, self.num_seeds, -1)  # (batch, num_seeds, d_model)
        # Rearrange Q for multihead attention: (num_seeds, batch, d_model)
        Q = Q.transpose(0, 1)
        # Prepare keys and values from X: (set_size, batch, d_model)
        X_t = X.transpose(0, 1)
        attn_output, _ = self.mha(Q, X_t, X_t)
        attn_output = self.ln(attn_output)
        # Transpose back: (batch, num_seeds, d_model)
        return attn_output.transpose(0, 1)

# ----------------------------
# Encoder Module with Dynamic PMA Block
# ----------------------------
class Encoder(nn.Module):
    def __init__(self, d_model, N, heads, frac=False, attn=True, compute_device=None, use_dynamic_pma=True):
        """
        ...
        use_dynamic_pma: if True, use Dynamic PMA with dynamic query generation.
        """
        super().__init__()
        self.d_model = d_model
        self.N = N
        ...
        self.attention = attn
        self.compute_device = compute_device
        self.use_dynamic_pma = use_dynamic_pma
        ...
        # Use the Dynamic PMA block for pooling.
        if self.use_dynamic_pma:
            self.pma = DynamicPMA(self.d_model, self.heads, num_seeds=1)
    ...
    def forward(self, src, frac):
        ...
        # Pool the set using the dynamic query based PMA block.
        if self.use_dynamic_pma:
...
class CrabNet(nn.Module):
    def __init__(self,
                 ...
                 residual_nn='roost'):
        super().__init__()
        self.avg = False  # With pooling via Dynamic PMA, we already aggregate the set.
    ...
    def forward(self, src, frac):
        ...
        x = self.encoder(src, frac)  # (batch, d_model) after dynamic PMA aggregation
