...
class ModelTrainer:
    ...
    def token_knockout_importance(self, src, frac, baseline_token=0):
        """
        Computes token-level (element-level) importance by knocking out
        each token in the input and measuring the change in model output.
        
        Args:
            src (torch.Tensor): Input tensor of token indices with shape [B, T].
            frac (torch.Tensor): Corresponding fractional tensor with shape [B, T].
            baseline_token (int): The token index to use as the baseline (e.g., 0).
        
        Returns:
            importance_scores (torch.Tensor): Tensor of shape [B, T] with importance scores.
        """
        self.model.eval()
        with torch.no_grad():
            ...
            for b in range(B):
                for t in range(T):
                    ...
                    # Difference metric: mean absolute difference on the b-th output
                    diff = torch.abs(original_output[b] - knockout_output[b]).mean()
                    importance_scores[b, t] = diff
        return importance_scores

    def feature_knockout_importance(self, src, frac, baseline_value=0.0):
        """
        Computes feature-level importance for the cbfv embedding features by knocking out
        one feature dimension at a time in the embedding matrix.
        
        Args:
            src (torch.Tensor): Input tensor of token indices with shape [B, T].
            frac (torch.Tensor): Corresponding fractional tensor with shape [B, T].
            baseline_value (float): The value to replace the feature with (e.g., 0.0).
        
        Returns:
            feature_importance (torch.Tensor): Tensor of shape [feat_size] containing
                                               importance scores for each feature.
        """
        self.model.eval()
        with torch.no_grad():
            original_output = self.model(src, frac)
            # Save original cbfv weights from the Embedder (shape: [vocab_size, feat_size])
            original_weights = self.model.encoder.embed.cbfv.weight.data.clone()
            ...
            for i in range(feat_size):
                ...
                # Temporarily update the cbfv embedding weights
    ...
    # -----------------------------------------------------------------
    # New functions that accept a DataLoader or CSV file path as input
    # -----------------------------------------------------------------
    def _get_loader(self, data_input, batch_size=128):
        """
        Internal helper: If data_input is a string, assume it's a CSV file path and load
        a DataLoader using DataHandler. Otherwise, assume it's already a DataLoader.
        """
        if isinstance(data_input, str):
            # Create a DataLoader for inference from the CSV file
