...
class GlobalRankedFeatureSelector(nn.Module):
    ...
    Uses a binary concrete (Gumbel–Sigmoid) relaxation to produce soft probabilities,
    which are then used as a mask. During training, the soft mask is used to allow gradients
    to flow back to self.logits.
    """
    def __init__(self, input_dim, k, init_temp=5.0, final_temp=0.1):
        """
        Args:
            input_dim (int): Number of features per token.
            k (int): Target number of features to keep (used for inference).
    ...
    def forward(self, x):
        ...
        Returns:
            x_masked: Tensor of shape [B, T, input_dim] after applying a soft mask.
        """
        # Sample Gumbel noise for each feature
        noise = -torch.log(-torch.log(torch.rand_like(self.logits) + 1e-20) + 1e-20)
        # Compute soft probabilities using the Gumbel–Sigmoid relaxation
        soft_probs = torch.sigmoid((self.logits + noise) / self.current_temp)
        # Use the soft probabilities directly as the mask so gradients flow.
        mask = soft_probs.view(1, 1, self.input_dim)
        return x * mask

    def update_temperature(self, epoch, total_epochs):
        # Exponential annealing schedule for temperature
        self.current_temp = self.init_temp * ((self.final_temp / self.init_temp) ** (epoch / total_epochs))
    
    def hard_mask(self):
        # For inference: compute a hard mask by thresholding the logits
