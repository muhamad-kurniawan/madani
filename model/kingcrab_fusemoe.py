...
class MoEElementFusion(nn.Module):
    def __init__(self,d_model,n_experts=16,top_k=4):
        ...
        self.expert_keys = nn.Parameter(torch.randn(1,n_experts,d_model))
        self.experts = nn.ModuleList([nn.Sequential(nn.Linear(d_model,4*d_model),
                                                    nn.GELU(),
                                                    nn.Linear(4*d_model,d_model))
                                       for _ in range(n_experts)])
        self.routers = nn.ModuleList()  # filled in Encoder depending on views
    def add_router(self):
        self.routers.append(nn.Linear(self.d_model,self.expert_keys.shape[1]))
    def _laplace_gate(self,h,router):
        B,L,D = h.shape
        keys = self.expert_keys.to(h.device).expand(B,-1,-1)
        dist = torch.cdist(h,keys,p=2)              # (B,L,n)
        logits = -(dist**2) + router(h)
        topv, topi = logits.topk(self.top_k, -1)
        w = torch.softmax(topv,-1)                  # (B,L,k)
        # expert outputs
        expert_out = torch.stack([E(h) for E in self.experts],2)  # (B,L,n,d)
        sel = torch.gather(expert_out,2,topi.unsqueeze(-1).expand(-1,-1,-1,D))
