...
###############################################################################
# New: MoE Feed-Forward module with Top-1 Gating
###############################################################################
class MoEFeedForwardTop1(nn.Module):
    """
    MoE feed-forward module with top-1 gating and load balancing regularization.
    ...
    def forward(self, x):
        B, T, d = x.shape
        gating_logits = self.gate(x)  # shape: [B, T, num_experts]
        ...
        # Compute importance per expert: sum over batch and tokens.
        importance = gating_scores.sum(dim=(0, 1))  # shape: [num_experts]
        load_loss = self.num_experts * (importance**2).sum() / (importance.sum()**2 + 1e-10)

        # Top-1 gating: select the single best expert per token.
        top1_scores, top1_experts = gating_scores.topk(1, dim=-1)
        top1_scores = top1_scores.squeeze(-1)      # [B, T]
        top1_experts = top1_experts.squeeze(-1)      # [B, T]

        out = torch.zeros_like(x)
        for e_idx in range(self.num_experts):
            mask = (top1_experts == e_idx)  # [B, T]
            if mask.any():
                selected_x = x[mask]  # [N, d]
                expert_out = self.experts[e_idx](selected_x)
                weight = top1_scores[mask].unsqueeze(-1)  # [N, 1]
...
class CustomTransformerEncoderMoELayer(nn.Module):
    """
    A Transformer Encoder layer that uses MoE (with top-1 gating) in its feed-forward block.
...
###############################################################################
# Mixed Encoder: First layer is MoE (with top-1 gating), remaining layers are standard.
...
class EncoderMoE(nn.Module):
    def __init__(self, d_model, N, heads, frac=False, attn=True,
        ...
        if self.attention:
            # Use the mixed encoder that applies MoE (with top-1 gating) only on the first layer.
